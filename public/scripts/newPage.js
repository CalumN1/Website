...
function drawCircle(event) {  //if in edit mode, need to add lines too
    ...
    newCircle.setAttribute('id', `node${nodeNo}`);
    ...
    newCircle.addEventListener('mouseover', (event) => {
        ...
        if (parseInt(newCircle.id.slice(4)) - 1 < nodeDetails.length)
            newText.innerHTML = nodeDetails[parseInt(newCircle.id.slice(4)) - 1].name;
        else
            newText.innerHTML = newCircle.id //node4
...
function redirectToText(nodeNo) {
    ...
    if (nodeNo - 1 < nodeDetails.length)
        URLtext = nodeDetails[nodeNo - 1].name;
    else
        URLtext = "newPage"
